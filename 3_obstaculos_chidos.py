...
# Estado = 0 El giro se calcula solo (detras del carro)
    if estado == 0:
        ...
        if medida < 1.0 or medidac < 1.0 or medidad < 1.0: #1.5
            # vel.publish(-600)
            estado = 1

        if data.ranges[30] < 1.5 or data.ranges[20]< 1.5:
            estado = 4
            # variablita = True
...
# Estado = 1 CAMBIO DE CARRIL
    if estado == 1:
        ...
        # if medida < 0.8 or medidac < 0.8 or medidad < 0.8:
        valores = data.ranges[300:309]
        valores = [x for x in valores if x < 1.2] #1.2
        if len(valores) >= 0:
            estado = 2
        # #     # vel.publish(0)
        # #     # time.sleep(0.5)
        ...
        # if data.ranges[270] < 1.0 or data.ranges[290] < 1.0:
            # print("bRRRR")o
            # vel.publish(0)
            # time.sleep(0.5)
            # estado = 2
